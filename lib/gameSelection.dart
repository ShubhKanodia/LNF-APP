        ...
        body: Stack(
          children: [
            Container(
                height: height,
                width: width,
                child: Image.asset("assets/GameSelection.png")),

            ///Todo : Need to change the background and text
            Positioned(
              top: height * 0.08,
              left: width * 0.04,
            ...
            Positioned(
              ...
              child: CircularOrangeButton(
                  ...
                  //     padding: const EdgeInsets.fromLTRB(60, 23, 60, 23),
            ...
            Positioned(
              ...
              child: CircularOrangeButton(
                  ...
                  onTap: () async {
                    var value = await userDocReference.get();
                    currentProgress.rewards = value.data()["rewards"];
                    var farmInfoFB = await userDocReference
                        .collection("BMCFarmFields")
                        .get();
                    var farmScoresFB = await userDocReference
                        .collection("BMCFarmScores")
                        .get();
                    List<FarmLandData> farmLandInfo =
                        List.filled(9, FarmLandData());
                    if (farmInfoFB.docs.isNotEmpty) {
                      farmInfoFB.docs.forEach((element) {
                        if (element.data()["deadline"] != null)
                          farmLandInfo[int.parse(element.id)] = FarmLandData(
                              vegetableCode: element.data()["vegetable"],
                    ...
                    farmScoresFB.docs.forEach((element) {
                      element.data().forEach((key, value) {
                        farmScores[element.id]
                            .elementAt(int.parse(key))
                            .selected = value;
                      });
                    });
                    if (farmInfoFB.docs.isEmpty && farmScoresFB.docs.isEmpty) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => BMCIntro(
                                  farmLandInfo: farmLandInfo,
                                ),
                            settings:
                                RouteSettings(name: 'Business Model Canvas')),
                      );
                    }
                    Navigator.push(
                      context,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
          boxShadow: [
            BoxShadow(
              color: light ? Color(0xFF9c27b0) : Color(0x66cb7703),
      ...
      child: ElevatedButton(
          child: Text(
            ...
            style: GoogleFonts.signika(
                color: light ? Color(0xffffffff) : Color(0xffffffff),
          ...
          style: ButtonStyle(
            shadowColor: MaterialStateProperty.all<Color>(
                light ? Color(0xFF9c27b0) : Color(0xFF9c27b0)),
